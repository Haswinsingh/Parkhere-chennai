...
const generateToken = (user) => {
    return jsonwebtoken_1.default.sign({ id: user._id, role: user.role }, env_1.ENV.JWT_SECRET, { expiresIn: env_1.ENV.JWT_EXPIRES_IN });
};
const register = async (req, res) => {
    try {
        const { name, email, phone, password, confirmPassword, role, 
        // Parking Holder Section A:
        numberOfVehicles, parkingPreference, securityPreference, 
        // Parking Holder Section B:
        landmark, cctvAvailable, gateAvailable, upiId, } = req.body;
        // Common validations
        ...
        let assignedRole = 'parking_needed';
        ...
        const newUser = await User_1.User.create({
            ...
            hostPreferences: assignedRole === 'parking_holder'
                ? {
