...
exports.getPaymentByBookingId = exports.confirmPayment = exports.createPayment = void 0;
const Booking_1 = require("../models/Booking");
const Payment_1 = require("../models/Payment");
const ParkingSpace_1 = require("../models/ParkingSpace");
const User_1 = require("../models/User");
const upiService_1 = require("../services/upiService");
...
const createPayment = async (req, res) => {
    try {
        ...
        const booking = await Booking_1.Booking.findById(bookingId).populate('parkingId', 'name');
        if (!booking) {
            res.status(404).json({
                success: false,
        ...
        const owner = await User_1.User.findById(booking.ownerId);
        ...
        if (method === 'upi') {
            const dynamicUpi = await (0, upiService_1.generateDynamicUpiPayment)({
                payeeUpiId,
                payeeName,
                amount: finalAmount,
                transactionReference,
                transactionNote: `ParkHere Booking ${booking.parkingId.name}`,
            });
            upiDetails = {
                payeeUpiId,
                payeeName,
        ...
        let payment = await Payment_1.Payment.findOne({ bookingId: booking._id });
        if (payment) {
        ...
        if (method === 'cash') {
            await (0, notificationService_1.sendNotification)({
                ...
                message: `${user.name} selected cash payment of ₹${finalAmount} for booking at ${booking.parkingId.name}. Please collect and confirm receipt.`,
                bookingId: booking._id,
            });
        }
        res.status(201).json({
...
const confirmPayment = async (req, res) => {
    try {
        ...
        let payment = paymentId
            ? await Payment_1.Payment.findById(paymentId)
            : await Payment_1.Payment.findOne({ bookingId });
        if (!payment) {
            res.status(404).json({
                success: false,
